#!/bin/bash
# professional-coaching-system-fix.sh - Script de Reingeniería del Overlay (Hextech Modular)
# Implementa: Hotkeys CTRL+F1/F2, Drag & Scale con persistencia, Bloqueo de Widgets, y Estética Hextech.

LOG_PREFIX="[HEXTECH-REFACTOR]"
NODE_PORT=3001
APP_NAME="lolmetamind-electron"

echo "=========================================================="
echo "$LOG_PREFIX 🚀 Aplicando Reingeniería Hextech y Correcciones Críticas"
echo "=========================================================="

# --- 1. CREACIÓN/MODIFICACIÓN DE ARCHIVOS DE CONTEXTO Y COMPONENTES ---
...
# 1.2. Crear src/context/ScaleContext.jsx (Nueva)
SCALE_CONTEXT_FILE="src/context/ScaleContext.jsx"
echo "$LOG_PREFIX 🆕 Creando $SCALE_CONTEXT_FILE: Contexto de Escala y Posición Persistente."
...
import { usePersistentIpc } from '../hooks/usePersistentIpc';
...
# 1.3. Crear src/components/widgets/DragAndScaleWidget.jsx (Nuevo Wrapper Hextech)
DRAG_WRAPPER_FILE="src/components/widgets/DragAndScaleWidget.jsx"
echo "$LOG_PREFIX 🆕 Creando $DRAG_WRAPPER_FILE: Wrapper Modular Hextech (Arrastre, Escala, Bloqueo)."
...
# 1.4. Modificar src/hooks/useInteractiveWidget.js (Nueva lógica de Hotkeys)
USE_INTERACTIVE_HOOK_FILE="src/hooks/useInteractiveWidget.js"
echo "$LOG_PREFIX 📝 Actualizando $USE_INTERACTIVE_HOOK_FILE: Manejo de estado de interactividad global (CTRL+F1/F2)."
...
# 1.5. Modificar src/app/overlay/page.jsx (Integración Completa)
OVERLAY_PAGE_FILE="src/app/overlay/page.jsx"
echo "$LOG_PREFIX 📝 Actualizando $OVERLAY_PAGE_FILE: Integración de ScaleContext, DragAndScaleWidget y lógica de TTS."
...
echo "$LOG_PREFIX ✅ Correcciones de código inyectadas con éxito."
echo "$LOG_PREFIX ----------------------------------------------------------"
...
echo "$LOG_PREFIX 🛠️ [PASO 3/4] Generando el Build de Next.js (necesario para las rutas de Electron en dev)..."
npm run build

if [ $? -ne 0 ]; then
    echo "$LOG_PREFIX ❌ ERROR: Fallo al generar el Build de Next.js. Abortando."
    exit 1
fi

echo "$LOG_PREFIX 🟢 [PASO 4/4] Iniciando el Sistema de Coaching en tiempo real (Modo Desarrollo Concurrente)..."
echo "----------------------------------------------------------"
echo "$LOG_PREFIX ⏳ ESPERE: Sincronizando Next.js y Electron. La aplicación iniciará en modo Pasivo (CTRL+F2)."
echo "----------------------------------------------------------"
